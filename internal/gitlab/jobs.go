...
// this cap surface as ErrTraceTooLarge.
const MaxTraceSize = 10 * 1024 * 1024

// ErrTraceTooLarge signals that a job's trace exceeded MaxTraceSize. The SDK
// does not expose HTTP Range support for the trace endpoint, so GetJobTrace
// returns this error rather than partial content when the cap is crossed.
...
// reports whether the underlying response exceeded MaxTraceSize so GetJobTrace
// can surface ErrTraceTooLarge.
